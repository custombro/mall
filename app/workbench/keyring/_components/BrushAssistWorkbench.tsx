...
const DEPLOY_MARKER = "DEPLOY_KEYRING_BRUSH_MAIN_20260415_02";
...
export default function BrushAssistWorkbench({
  ...
  const moveBrush = (event: ReactPointerEvent<SVGSVGElement>) => {
    ...
    setStrokes((prev) => {
      ...
      const next = [...prev];
      next[next.length - 1] = { points: [...next[next.length - 1].points, point] };
      return next;
    });
  };
      ...
      <div className="mx-auto w-full max-w-[1580px] px-4 py-4">
        <header className="sticky top-3 z-20 rounded-[28px] border border-black/10 bg-white px-5 py-4 shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            ...
            <div className="grid min-w-[420px] gap-2 sm:grid-cols-3">
              {["1 파일 올리기", "2 캐릭터 칠하기", "3 칼선 확인"].map((label) => (
                <div key={label} className={["rounded-2xl border px-4 py-3 text-center text-sm font-extrabold", currentStage === label ? "border-neutral-950 bg-neutral-950 text-white" : "border-black/10 bg-[#f7f4ed] text-neutral-700"].join(" ")}>
                  {label}
                </div>
              ))}
            </div>
          ...
          <div className="mt-4 grid gap-2 lg:grid-cols-[1fr_1fr_1fr_auto]">
            <div className="rounded-2xl border border-black/10 bg-[#f7f4ed] px-4 py-3"><div className="text-[11px] font-bold tracking-[0.18em] text-neutral-500">현재 단계</div><div className="mt-1 text-base font-extrabold">{currentStage}</div></div>
            <div className="rounded-2xl border border-black/10 bg-[#f7f4ed] px-4 py-3"><div className="text-[11px] font-bold tracking-[0.18em] text-neutral-500">현재 상태</div><div className="mt-1 text-base font-extrabold">{currentStatus}</div></div>
            <div className="rounded-2xl border border-black/10 bg-[#f7f4ed] px-4 py-3"><div className="text-[11px] font-bold tracking-[0.18em] text-neutral-500">다음 행동</div><div className="mt-1 text-base font-extrabold">{nextAction}</div></div>
            <div className="flex flex-wrap items-center justify-end gap-2 text-xs">
              {showAuxLinks ? <><Link href="/workbench/keyring/brush-assist-lab" className="rounded-full border border-black/10 bg-[#f7f4ed] px-3 py-2 font-bold">작업 실험실</Link><Link href="/workbench/keyring/brush-assist-demo" className="rounded-full border border-black/10 bg-[#f7f4ed] px-3 py-2 font-bold">간단 데모</Link></> : null}
        ...
        <section className="mt-4 grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)_320px]">
          <aside className="rounded-[28px] border border-black/10 bg-white p-4 shadow-sm xl:sticky xl:top-[205px] xl:self-start">
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()} className="mt-4 flex h-16 w-full items-center justify-center rounded-2xl bg-neutral-950 px-4 text-[18px] font-extrabold text-white shadow-[0_8px_20px_rgba(0,0,0,0.14)]">이미지 올리기</button>
            <div className="mt-4 max-h-[calc(100vh-420px)] space-y-2 overflow-y-auto pr-1">
              {uploads.length === 0 ? <div className="rounded-2xl border border-dashed border-black/15 p-4 text-sm text-neutral-500">아직 올린 파일이 없습니다</div> : uploads.map((item) => {
                const active = activeId === item.id;
                return (
                  <button key={item.id} type="button" onClick={() => { setActiveId(item.id); setStrokes([]); setIsConfirmed(false); }} className={["w-full rounded-2xl border px-3 py-3 text-left transition", active ? "border-neutral-950 bg-neutral-950 text-white" : "border-black/10 bg-[#faf9f6] text-neutral-900"].join(" ")}>
          ...
          <section className="rounded-[28px] border border-black/10 bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 overflow-hidden rounded-[28px] border border-black/10 bg-[#0f172a]">
              <svg viewBox="0 0 100 100" className="h-[min(54vh,620px)] w-full touch-none" onPointerDown={startBrush} onPointerMove={moveBrush} onPointerUp={stopBrush} onPointerCancel={stopBrush} onPointerLeave={stopBrush}>
                ...
                {activeUpload?.previewUrl ? <image href={activeUpload.previewUrl} x="0" y="0" width="100" height="100" preserveAspectRatio="none" opacity="0.96" /> : null}
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <button type="button" onClick={clearBrush} className="rounded-2xl border border-black/10 px-4 py-3 text-sm font-bold">칠한 표시 지우기</button>
              <div className="rounded-2xl border border-black/10 bg-[#f7f4ed] px-4 py-3 text-sm text-neutral-600">캐릭터 중심으로 러프하게 칠하고 바로 칼선 결과를 보세요.</div>
            </div>
          </section>
          ...
          <aside className="rounded-[28px] border border-black/10 bg-white p-4 shadow-sm xl:sticky xl:top-[205px] xl:self-start">
            ...
            <div className="mt-3 rounded-2xl border border-black/10 bg-[#f7f4ed] p-4 text-sm font-bold text-neutral-700">{!activeUpload ? "파일을 올리면 여기서 바로 결과를 볼 수 있어요." : isConfirmed ? "칼선 확인을 마쳤습니다. 주문으로 넘어가세요." : canConfirm ? "칼선 기준 준비 완료" : "캐릭터 부분을 러프하게 칠해 주세요."}</div>
            <button type="button" onClick={() => { if (!canConfirm) return; setIsConfirmed(true); }} className="mt-4 flex h-16 w-full items-center justify-center rounded-2xl bg-neutral-950 px-4 text-[18px] font-extrabold text-white shadow-[0_8px_20px_rgba(0,0,0,0.14)] disabled:cursor-not-allowed disabled:bg-neutral-300" disabled={!canConfirm}>{isConfirmed ? "칼선 확인 완료됨" : "칼선 확인 완료"}</button>
          </aside>
        </section>
      </div>
    </main>
